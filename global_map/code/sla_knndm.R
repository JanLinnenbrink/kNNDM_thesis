...
# pre-processing only necessary when using the original prediction map
...
#ggsave("figures/sla_map.pdf",sla_map, width=12, height=6)
...
# train and evaluate model based on random CV
...
# calculate W stat for random CV
...
W_rand <- twosamples::wass_stat(Gij, Gjstar)
sla_rand <- list(Gj=Gj, Gij=Gij, Gjstar=Gjstar)

message(paste("W stat with random CV:", round(W_rand), "\nW stat with kNNDM CV:", round(sla_kndm$W)))
...
# plot tpoints colored by CV-folds
sla <- rbind(sla,sla)
sla$CV <- NaN
...
upper_row <- plot_grid(sla_map_rand,sla_map_kndm,nrow=1)
plot_grid(upper_row, ecdfs, nrow=2)
