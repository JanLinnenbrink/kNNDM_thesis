...
knndm <- function(tpoints, modeldomain = NULL, ppoints = NULL,
  ...
  if(testks$p.value >= 0.05){
    ...
    for(nk in clustgrid$nk){
      ...
      if (categorical == TRUE) {
        cat_var <- "nsw16"
        tabclust <- as.data.frame(table(tpoints[[cat_var]], clust_nk))
        tabclust <- split(tabclust, tabclust$Var1)
        tabclust <- lapply(tabclust, function(x) x[order(x$Freq, decreasing=T),])
        tabclust <- lapply(tabclust, function(x) {x$clust <- NA; return(x)})
