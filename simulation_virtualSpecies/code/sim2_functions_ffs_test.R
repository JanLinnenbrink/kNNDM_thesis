...
AOA_perc <- function(model, indexTrain=NULL, indexTest=NULL, predictor_stack) {
  ...
  list(data.frame(inside_AOA = AOAp[[2]], outside_AOA = AOAp[[1]]), AOA, AOA_res)
}
...
calc_diff <- function(cntrl, what, traindf, predictor_names,outcome_rast,
  ...
  quiet(mod <- train(traindf[,predictor_names], traindf$outcome, method="rf",
  ...
  true_rmse_ffs <- predict(object=predictor_stack,model=mod, na.rm=TRUE) |> 
    base::`-` (outcome_rast) |> 
    base::`^`(2) |> 
    mean() |> 
    sqrt() |> 
  ...
  diff_to_normal <- predict(object=predictor_stack,model=model_wo, na.rm=TRUE) |> 
    base::`-` (outcome_rast) |> 
    base::`^`(2) |> 
    mean() |> 
    sqrt() |> 
  ...
  AOA_improvement <- AOA_perc(model_wo, predictor_stack=predictor_stack)[[1]][[1]] |> 
    base::`-`(AOA_spat[[1]][[1]])
  ...
  ffs_stats <- data.frame(n_predictors=n_predictors_ffs, 
                          ...
                          improvement_rmse=diff_to_normal[[1]],
