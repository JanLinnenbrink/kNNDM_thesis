...
wclim <- terra::project(rast(wclim), res = 5000, y = "epsg:25830") |> stack()
# levelplot(stretch(wclim))
...
st_write(spoly, dsn="data/species_vdata.gpkg", layer="sampling_polygon", append = TRUE)
st_write(wgrid, dsn="data/species_vdata.gpkg", layer="landscape_grid", append = TRUE)
writeRaster(wclim,  "data/species_stack.grd", format="raster", overwrite=TRUE)
